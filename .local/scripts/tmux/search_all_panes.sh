...
done < <(tmux list-panes -a -F "#{pane_id} #{window_name}") | fzf -d " " --with-nth=3.. --height=0 --cycle -i -e --preview="tmux capture-pane -e -p -S -$HISTORY_LIMIT -t {1} | grep -i --color=always -C 15 "{q}"")
